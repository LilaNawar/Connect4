...
// CONTROLLER 
// checks hortizontal DONE
function checkHorizontal(newRow, column){
  column = Number(column)    

  //3 left
  if (column>=3 ){
    if((turn ==boardMatrix[newRow][column-1]) && (turn == boardMatrix[newRow][column-2])&&(turn == boardMatrix[newRow][column]) && (turn == boardMatrix[newRow][column-3])){
      winner = turn;
  ...
  if (column>= 2 && column <=5){
    if ((turn ==boardMatrix[newRow][column-1]) &&(boardMatrix[newRow][column+1] == turn) && (boardMatrix[newRow][column]== turn) &&  (boardMatrix[newRow][column-2] == turn)){
      winner = turn;
      showMessage(`Congratulations ${winner}!`);
      return 
      }
    ...
    // three right
  if (column<=3){
    if ((turn == boardMatrix[newRow][column+1]) &&(boardMatrix[newRow][column+2] == turn) && (boardMatrix[newRow][column]== turn) &&  (boardMatrix[newRow][column+3] == turn)){
      winner = turn;
      showMessage(`Congratulations ${winner}!`);
      return 
      }
...
// check diagonal right  NOT DONE
function checkDiagonalRight(newRow, column){
  column = Number(column)
  //1 up 2 down right DONE
  if (column<=4 && column>=1 && newRow<=3 && newRow >=1){
    if((boardMatrix[newRow][column]== turn)&&(boardMatrix[newRow+1][column-1]== turn) && (boardMatrix[newRow+2][column-2]== turn) && (boardMatrix[newRow-1][column+1]== turn)){
        winner = turn;
        showMessage(`Congratulations ${winner}!`);  
        return
    }
  ...
  // not working enters, but doesn't evaluate to true
  if(column<=3 && column>=0 && newRow<3 && newRow >=0){
   //3 down right  DONE
    if((boardMatrix[newRow][column]== turn) &&(boardMatrix[newRow+1][column+1]== turn) && (boardMatrix[newRow+2][column+2]== turn) && (boardMatrix[newRow+3][column+3]==turn)){
        winner = turn;
        showMessage(`Congratulations ${winner}!`);
        return
      }
  }
  // 3 up right WORKS DONE
...
function checkDiagonalLeft(newRow, column){
  ...
  if(newRow <= 4 && newRow>=2 && column>=2  && column <= 5){
    //1 down 2 up left  DONE
    if((boardMatrix[newRow][column]== turn)&&(boardMatrix[newRow+1][column+1]== turn) && (boardMatrix[newRow-2][column-2]== turn) && (boardMatrix[newRow-1][column-1]== turn)){
        winner = turn;
        showMessage(`Congratulations ${winner}!`);  
    }
}
     // 1 up 2 down DONE WORKS
  if(newRow <= 3 && newRow>=1 && column>=0  && column <= 4){
    if((boardMatrix[newRow][column]== turn) &&(boardMatrix[newRow-1][column-1]== turn) && (boardMatrix[newRow+1][column+1]== turn) && (boardMatrix[newRow+2][column+2]==turn)){
        winner = turn;
        showMessage(`Congratulations ${winner}!`);
      }
}
    // // 3 left down DONE WORKS
...
// checks vertical matches DONE
function checkVertical(newRow, column){
  column = Number(column)
  if (newRow <3 ){
    if((turn ==boardMatrix[newRow+1][column]) && (turn == boardMatrix[newRow+2][column])&& (turn == boardMatrix[newRow+3][column])&& (turn == boardMatrix[newRow][column])){
      winner = turn; 
      showMessage(`Congratulations ${winner}!`);
      return winner
}
...
function checkWinner(newRow, col){
  if (winner != "" && winner !="tie")
    {return}
  // check to see if clicked position is already 
  if (winner === ""){
...
function insertImage(newRow, collum){
  ...
  if (turn == "player1"){
    img.src = "https://toppng.com/uploads/preview/western-cowboy-cowgirl-free-clip-art-lasso-png-lasso-11563494262giljsuynv1.png"; 
    	document.getElementById(newId).appendChild(img);
  }
  // cowboy hat
  else{
    img = document.createElement('img'); 
    img.src = "https://clipartcraft.com/images/cowboy-hat-transparent-emoji-9.png"; 
	  document.getElementById(newId).appendChild(img);
  }
}
...
//bubbled down to the lowest empty spot in that column
